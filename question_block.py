...
class QuestionsBlock:
    def __init__(self, client, user, file_name):
        ...
        with open(f'{os.getcwd()}\\resources\\questions\\{file_name}_{self.lang}.json', mode="r", encoding="utf8") as f:
    ...
    def print_question(self, concrete_question):
        ...
        self.client.send_message(self.user_id, text=QUESTION.format(text=text), reply_markup=markup_inline)

    def get_answer(self, data, client_storage):
        if data == "true":
            self.client.send_message(self.user_id, text=TEST_PASS_ANSWER)
            self.true_counter += 1
        else:
            self.client.send_message(self.user_id, text=TEST_FAIL_ANSWER)
